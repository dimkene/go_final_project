...
import (
	...
	"github.com/dimkene/go_final_project/internal/config"
	"github.com/dimkene/go_final_project/internal/db/repo"
	"github.com/dimkene/go_final_project/internal/models"
)
...
func (a *Api) UpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	err = a.repo.UpdateTask(parseBody)
	if err != nil {
		log.Println("error:", err)
		RenderApiErrorAndResponse(w, fmt.Errorf(InternalServerError), http.StatusInternalServerError)
		return
...
func (a *Api) SigninHandler(w http.ResponseWriter, r *http.Request) {
	...
	secret := []byte(a.config.SecretKey)
	hashedUserPassword := HashPassword([]byte(reqBody.Password), secret)
	hashedEnvPassword := HashPassword([]byte(a.config.AppPass), secret)
	...
	tokenValue, err := createToken(reqBody.Password, a.config.SecretKey)
	if err != nil {
		RenderApiErrorAndResponse(w, fmt.Errorf(InternalServerError), http.StatusInternalServerError)
	}
